...
class UAVenv(gym.Env):
    ...
    def user_location_rb(self):
        ...
        for step_count in range(step):
            for i in range(len(HOTSPOTS)):
                for j in range(USER_DIS):
                    temp_loc_r = random.uniform(-(1/3.5)*self.COVERAGE_XY, (1/3.5)*self.COVERAGE_XY)
        ...
        self.USER_LOC = np.concatenate((USER_LOC, temp_loc))
        np.savetxt('UserLocation.txt', USER_LOC_2D, fmt='%.3e', delimiter=' ', newline='\n')
    ...
    def step(self, action, info_exchange_lvl):
        ...
        for i in range(self.NUM_UAV):
            ...
            if self.state[i, 0] < 0 or self.state[i, 0] > self.GRID_SIZE or self.state[i, 1] < 0 or self.state[
                    i, 1] > self.GRID_SIZE:
            ...
            for l in range(self.NUM_USER):
                dist_u_uav[i, l] = math.sqrt((self.u_loc[l, 0] - (self.state[i, 0] * self.grid_space))**2 +
                                             (self.u_loc[l, 1] - (self.state[i, 1] * self.grid_space))**2)
        ...
        for k in range(self.NUM_UAV):
            for l in range(self.NUM_UAV):
                dist_uav_uav[k, l] = math.sqrt(((self.state[l, 0] - self.state[k, 0]) * self.grid_space)**2 +
                                               ((self.state[l, 1] - self.state[k, 1]) * self.grid_space)**2)
        ...
        for k in range(self.NUM_UAV):
            ...
            for l in range(self.NUM_UAV):
                if k != l:
                    temp_penalty = max(0, ((2 * self.coverage_radius - dist_uav_uav[k, l]) /
                                           (2 * self.coverage_radius)) * max_overlap_penalty)
        ...
        for i in range(self.NUM_USER):
            ...
            if dist_u_uav[close_uav, i] <= self.coverage_radius:
                # UAV - User distance within the coverage radius then only connection request
        ...
        for i in range(self.NUM_UAV):
            ...
            # Contains user index with closest 2D distance value (out of requested user)
