...
class DQL:
    ...
    def epsilon_greedy(self, state):
        ...
        if temp <= self.epsilon_thres:
            ...
            if args.covered_user_as_input:
                ...
                x = 3
            Q_values = self.main_network(state[:, 0:x])
    ...
    def train(self,batch_size, dnn_epoch):
        for k in range(dnn_epoch):
            ...
            # To input the covered users by that particular UAV as input to the neural network // or just position otherwise
            if args.covered_user_as_input:
                x = 2
            else:
                x = 3
            ...
            Q_next = self.target_network(next_state[:,0:x]).detach()
            ...
            Q_main = self.main_network(state[:,0:x]).gather(1, action).squeeze()
            ...
            for t in range(100):
                ...
                for k in range(NUM_UAV):
                    ...
                    if args.covered_user_as_input:
                        ...
                        x = 3
                    Q_values = UAV_OB[k].main_network.forward(state[0, 0:x].float())
                    best_next_action = Q_values.max(1)[1].view(1,1)
